...
if ($action === 'get_analytics') {
    ...
    if ($res && $row = $res->fetch_assoc()) {
        ...
        $url = "https://graph.facebook.com/v23.0/$waba_id?fields=conversation_analytics.start($start_ts).end($end_ts).granularity(DAILY),analytics.start($start_ts).end($end_ts).granularity(DAILY),message_templates,template_analytics.start($start_ts).end($end_ts).granularity(DAY)";
